...
public:
    ...
    void Draw(cv::Mat frame) const
    {
        ...
        cv::line(frame, Ptf2i(m_pt1), Ptf2i(m_pt2), cv::Scalar(0, 255, 0), 6, cv::LINE_8, 0);
...
private:
    ...
    cv::Mat RGBFrame;
    QImage img;
//    vector<CRegion> regon;

signals:
    void send_processed_image(const QImage &image);
    void send_on_video_stop();
    void send_on_video_start();
    void send_on_video_end();
...
public:
    bool load_video(std::string filename);
    void play_video();
    void pause_video();
    void stop_video();
    bool is_stopped() const;
    bool isStopped2() const;
    void set_current_frame(int frameNumber);

    double get_frame_rate();
    double get_current_frame();
    double get_number_of_frames();
//    changes and transitions
    void add_counter_line(const RoadLine& newLine);
    bool get_counter_line(unsigned int lineUid, RoadLine& line);
    bool remove_counter_line(unsigned int lineUid);
    Detector CarDetect;
protected:
...
public:
    std::string video_to_play;
signals:
    void send_line_data(unsigned int&, std::string, int);
    void send_count_data(int, int, QString);
    void send_count_classification_data();

public:
    int number_of_count_lines;
    std::map<int, std::map<int, QString>>count_data;
    bool _show_counter_lines = false;
private:
    void initialize_count_data();
    void update_data();
